...
const createProjectAction = async project => {
  ...
  fs.writeFile(languagePath, JSON.stringify({ language }), async err => {
    if (!err) {
      ...
      try {
        ...
        spinner.succeed()
        console.log(symbols.success, chalk.green(`Successfully created project ${project}.`))
...
const addRStoreAction = async () => {
  ...
  if (!isAddPath) {
    const { inputAddPath } = await inquirer.prompt({
      type: 'input',
      message: 'Please enter manually(Example: pages/index).',
      name: 'inputAddPath'
    })
    dest = inputAddPath
  }
    ...
    if (!err) {
      ...
      fs.mkdir(`${dest}/store`, err => {
        if (!err) {
          ...
          console.log(symbols.success, chalk.green(`Success: The store module is created in ${dest}.`))
        } else {
          console.log(chalk.red(`Error: The file already exists, mkdir ${dest}.`))
        }
      })
    } else {
      console.log(chalk.red(err))
